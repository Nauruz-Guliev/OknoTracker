...
fun SettingItem.toDto(): SettingItemDto =
    SettingItemDto(key = key.name, isChecked = isChecked, title = title)

fun List<SettingItem>.toDtoList(): List<SettingItemDto> = map(SettingItem::toDto)

fun SettingItemDto.toDomain(): SettingItem =
    SettingItem(key = SettingKey.valueOf(key), isChecked = isChecked, title = title)
